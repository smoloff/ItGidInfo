...
const f7 = () => {
    ...
    let iArr = Array.from(input);
    let check = new Set(iArr);
    if (iArr.length == check.size && iArr.length > 6) {
        toOut('.out-7', 1);
    } else {
        toOut('.out-7', 0);
    }
...
const f10 = (out_set, elem) => {
    ...
    document.querySelector(elem).innerText = str;
 }
